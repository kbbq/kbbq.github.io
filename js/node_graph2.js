...
var cy2 = cytoscape({
  ...
  elements: {
    ...
    edges: [
     ...
     { data: { source: 'jason', target: 'gid' } }, //veto
     { data: { source: 'andrew', target: 'gid' } },
     { data: { source: 'xi', target: 'jp' } },
     { data: { source: 'eric', target: 'jp' } },
     { data: { source: 'justin', target: 'jp' } },
     { data: { source: 'andrew', target: 'kevin' } }, //veto
     { data: { source: 'jp', target: 'sirek' } }, //veto
     { data: { source: 'brian', target: 'gid' } }, //veto
     { data: { source: 'jp', target: 'xi' } },
     { data: { source: 'xi', target: 'andrew' } }, //veto
     { data: { source: 'andrew', target: 'jp' } }, //veto
     { data: { source: 'kevin', target: 'andrew' } }, //veto
     { data: { source: 'jp', target: 'andrew' } }, //veto
...
var tableString = "";
  for(i=0; i<nodes.length; i++){
    ...
    var dcI     = cy2.$().dc({ root: nodes[i], directed: true }).indegree;
    var dcO     = cy2.$().dc({ root: nodes[i], directed: true }).outdegree;
    var dc      = cy2.$().dc({ root: nodes[i] }).degree;
    var cc      = cy2.$().cc({ root: nodes[i] });

    tableString += "<tr><td>"+name+"</td><td>"+degree+"</td><td>"+dcI+"</td><td>"+dcO+"</td><td>"+dc+"</td><td>"+cc+"</td></tr>";

    /*
    console.log("Name: "+nodes[i].data().name);
    console.log("Degree: "+nodes[i].degree());
    console.log("Id: "+nodes[i].id());
    console.log("DC: "+cy2.$().dc({ root: nodes[i] }).degree);
    console.log("CC: "+cy2.$().cc({ root: nodes[i] }));
    console.log("-----");
    //console.log(cy2.dc(nodes[i].id()).degree);
    */
    /*
    console.log("DC T: "+cy2.$().dc({ root: nodes[i], directed: true }).indegree);
    console.log("DC T: "+cy2.$().dc({ root: nodes[i], directed: true }).outdegree);
    console.log("-----");
    */
  }
  document.getElementById("cy2TableBody").innerHTML = tableString;
